...
int main(int argc, char **argv) {
    ...
    int conejosIniciales =
        0;  // Se almacenaran los conejos ingresados por el usuario
    ...
    if (lista_partidas != NULL) {
        ...
        recuperarPartida =
            (Bool)preguntayn("Existen partidas guardadas, quieres cargar una?");
        ...
        if (recuperarPartida) {
            ...
            do {
                ...
                clearStdin();
                scanf("%i", &i);
                if (i < 1 || i > lista_partidas_num)
                    printMsjError("Partida no valida!");
            } while (i < 1 || i > lista_partidas_num);
            sprintf(partida_ruta, "%s%s%i%s", FOLDER_PARTIDAS,
                    FILE_PARTIDA_PREFIX, i - 1, FILE_PARTIDA_EXT);
            tablero =
                cargarPartida(partida_ruta, &m, &n, &conejosIniciales,
                              &conejosVivos, &nivel, &puntaje, &trampolines);
            if (tablero == NULL) {
                printMsjError(
                    "Error: No se logro conseguir memoria para el tablero!");
                return 1;
            }
        }
    }
    ...
    // 	clearScr();
    // 	recuperarPartida = False;
    // 	recuperarPartida = (Bool)preguntayn("Existe una partida guardada, desea
    // cargarla?");

    // 	// Si la respuesta fue afirmativa recupero el estado del juego
    // 	if(recuperarPartida)
    // 		tablero = cargarPartida(FILE_PARTIDA, &m, &n, &conejosIniciales,
    // &conejosVivos, &nivel, &puntaje);
    ...
    for (;;) {
        ...
        // println("Conejos Vivos: %i - Nivel: %i - Puntaje: %i", conejosVivos,
        // nivel + 1, puntaje);
        printf("%sConejos Vivos: %i%s", COLOR_ROJO_CLARO_S, conejosVivos,
               ESTILO_NORMAL_S);
        printf(" - %sNivel: %i%s", COLOR_VERDE_CLARO_S, nivel + 1,
               ESTILO_NORMAL_S);
        printf(" - %sPuntaje: %i%s\n", COLOR_MARRON_CLARO_S, puntaje,
               ESTILO_NORMAL_S);
        // Pido movimiento al usuario
        siguienteMovimiento = pedirSiguienteMovimiento();
        // Establesco el movimiento como no valido hasta que se demuestre lo
        // contrario
        movValido = False;
        ...
        if (siguienteMovimiento >= 1 && siguienteMovimiento <= 9) {
            ...
            movValido = ejecutarMovimientoZanahoria(siguienteMovimiento,
                                                    tablero, m, n, nivel)
                            ? True
                            : False;
            if (!movValido) {
                ...
                gameOver =
                    (ejecutarMovimientoConejos(tablero, tableroCopia, m, n,
                                               &conejosVivos, &puntaje) == 0)
                        ? True
                        : False;
            }
            ...
            gameOver = (ejecutarMovimientoConejos(tablero, tableroCopia, m, n,
                                                  &conejosVivos, &puntaje) == 0)
                           ? True
                           : False;
            ...
            lista_partidas_num = longitud_lista(lista_partidas);
            clearScr();
            if (lista_partidas_num)
                printf(
                    "Puedes guardar unar partida nueva o sobreescribir una "
                    "existente.\n\n");
            if (lista_partidas_num &&
                preguntayn("Deseas sobreescribir una partida?")) {
                clearScr();
                ...
                sprintf(partida_ruta, "%s%s%i%s", FOLDER_PARTIDAS,
                        FILE_PARTIDA_PREFIX, i - 1, FILE_PARTIDA_EXT);
                pedirCadena("\nBreve descripcion partida", partida_nombre,
                            NODO_NOMBRE_MAX_LENGTH);
                if (guardarPartida(partida_ruta, tablero, m, n, partida_nombre,
                                   conejosIniciales, conejosVivos, nivel,
                                   puntaje, trampolines)) {
                    modificar_valor_posicion(lista_partidas, i - 1,
                                             partida_nombre);
                ...
                sprintf(partida_ruta, "%s%s%i%s", FOLDER_PARTIDAS,
                        FILE_PARTIDA_PREFIX, lista_partidas_num,
                        FILE_PARTIDA_EXT);
                clearScr();
                pedirCadena("Breve descripcion partida", partida_nombre,
                            NODO_NOMBRE_MAX_LENGTH);
                if (guardarPartida(partida_ruta, tablero, m, n, partida_nombre,
                                   conejosIniciales, conejosVivos, nivel,
                                   puntaje, trampolines)) {
                    lista_partidas =
                        insertar_por_cola(lista_partidas, partida_nombre);
                    printMsjOkPausa("Partida Guardada!");
                }
            }
            ...
            // 	if(preguntayn("Ya existe una partida guardada, desea
            // sobreescribirla?")){ 		if(guardarPartida(FILE_PARTIDA,
            // tablero, m, n, conejosIniciales, conejosVivos, nivel, puntaje))
            // 			printMsjOkPausa("Partida Guardada!");
            // 	}
            // }else{
            // 	if(guardarPartida(FILE_PARTIDA, tablero, m, n, conejosIniciales,
            // conejosVivos, nivel, puntaje)) 		printMsjOkPausa("Partida
            // Guardada!");
            // }
        ...
        if (movValido) {
            ...
            if (conejosVivos == 0) {
                ...
                conejosVivos =
                    conejosIniciales +
                    (redondeoEntero(conejosIniciales * TASA_AUMENTO_CONEJOS) *
                     nivel);
                ubicarConejosIniciales(tablero, m, n, conejosVivos);
                // Ubico los Trampolines
                if (nivel >= NIVEL_TRAMPOLINES_START) {
                    trampolines += nivel == NIVEL_TRAMPOLINES_START
                                       ? TRAMPOLINES_INICIALES
                                       : trampolines * TASA_AUMENTO_TRAMPOLINES;
