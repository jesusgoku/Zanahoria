 ...
 *
 * Aqui encontraras las funciones que son de utilidad para este juego y que
 * dificilmente podran ser utilizadas en otros proyectos a menos que sean muy
 * similares.
 *
 * @file zanahoria.h
 * @version 0.1
 ...
 * @{
 */
#define FOLDER_PARTIDAS \
    "partidas/" /*!< Carpeta donde se guardas las partidas */
#define FILE_PARTIDA_PREFIX                                            \
    "partida-"                  /*!< Prefijo para las partidas guardas \
                                 */
 ...
 * para la cantidad de conejos ingresadas por el usuario, cosa de que el juego
 * tenga sentido. Se ha establecido arbitrariamente al 10% del tamaño total de
 * casillas disponibles en el tablero esto pensando en que a medida de que
 * avanze el juego la cantidad de conejos ira aumentando.
 *
 ...
 * @param ci puntero a entero donde se almacenara el numero de conejos iniciales
 * ingresado.
 *
 */
 ...
 *
 * Se ha establecido arbitrariamente que las dimensiones del tablero deben de
 * ser impar con el objeto de que haya un solo centro en lugar de cuatro cuando
 * las dimensiones del tablero es par.
 *
 * Tambien se ha establecido como tamaño minimo de tablero el numero 5 ya que es
 * el minimo aceptable para que caiga la zanahoria en el centro y un 10% de
 * conejos y se pueda desarrollar un juego.
 *
 * Como comentario adicional se establecen la cantidad de filas y columnas por
 * separado, aun cuando para este juego la dimension del tablero es cuadrada,
 * pero he decidido dejarla asi para que pueda ser modificado facilmente en caso
 * de que se desee que no sea asi.
 *
 ...
 *
 * llena todo las casillas del tablero con el caracter representado por la
 * constante CELDA_VACIA
 *
 ...
 *
 * Esta es una version mejorar la version anterior que muestra el tablero de una
 * forma mas organizada a la vista al simular verdaderamente un tablero y sus
 * casillas por donde se desplazan la zanahoria y los conejos que quieren
 * devorarla. Tiene la desventaja de ocupar mas espacio para su representacion,
 * lo que limita el tamano de los tableros posibles.
 *
 * @param tablero puntero a la matriz que representa al tablero
 ...
 * @param filas entero con la cantidad de filas del tablero
 * @param columnas entero con la cantidad de columnas del tablero
 * @param coordZF puntero a entero donde se almacenara la coordena fila de la
 * zanahoria
 * @param coordZC puntero a entero donde se almacenara la coordena columna de la
 * zanahoria
 */
 ...
 *
 * Se encarga de validar que al menos sea un comando de juego valido o lo vuelve
 * a pedir
 *
 * @return devuelve un entero que representa el movimiento ingresado por el
 * usuario
 */
 ...
 *
 * Ademas se debe validar que para que el movimiento sea valido, no exista el
 * riesgo de que en la casilla a donde se va a mover vaya a ser comido. Por eso
 * se utilizan dos funciones auxiliares para verificar las dos vecindades de si
 * hay algun potencial conejo que pueda hacer invalida la jugada. Por ese motivo
 * se pasa el nivel que a primeras podria parecer inecesario pero a como los
 * trampolines solo comienzan a aparecer desde el 2do nivel y estos son lo que
 * permiten a los conejos saltar de a dos espacios, no tiene sentido gastar
 * tiempo llamando a la funcion que se encarga de revisar la segunda vecindad si
 * no hay riesgo aun.
 *
 * Para realizar el ahorro comentado en el parrafo anterior se aprovecha la
 * evaluacion de expresiones logicas de C por cortocircuito evaluando si el
 * nivel es menos que el nivel en que comienzan a aparecer los trampolines O
 * verificar la vecindad, y ya que hasta que se alcanze el nivel la primera
 * expresion siempre dara verdadero, se ya sabe que la expresion es verdadera y
 * no ejecutara la funcion para revisar la vecindad. No asi cuando se alcanza el
 * nivel para los trampolines donde la primera expresion dara falso y C se vera
 * obligado a evaluar la seguna expresion, osea la funcion que verifica el
 * segundo cuandrante para conocer el valor de verdad de la expresion,
 * dependiendo ahora exclusivamente de ella el valor de la expresion.
 *
 ...
 *
 * @return Devuelve 0 si el movimiento no es valido, y 1 si lo es y la mueve
 * hasta la posicion
 */
 ...
 *
 * @param tablero puntero a la matriz que representa al tablero
 * @param tableroCopy puntero a la matriz que representa a una copia del tablero
 * para poder realizar los movimientos
 * @param filas entero con la cantidad de filas del tablero
 * @param columnas entero con la cantidad de columnas del tablero
 * @param conejosVivos puntero a entero donde se almacena la cantidad de conejos
 * vivos que se disminuira en caso de colision de conejos
 * @param puntaje puntero a entero donde se lleva el puntaje que ira aumentando
 * en caso de colision de acuerdo a los establecido en la constante
 * PUNTAJE_CHOQUE
 *
 * @return Devuelve 0 en caso de que un conejo haya caido sobre la zanahoria, en
 * caso contrario retorna 1
 */
int ejecutarMovimientoConejos(char **tablero, char **tableroCopy,
...
/**
 * @brief Verifica la vencindad de la zanahoria para verificar si hay peligro al
 * ubicarse hay.
 *
 * @param tablero puntero a la matriz que representa al tablero
 * @param m entero con la cantidad de filas del tablero
 ...
 * @param salto entero, si valor es 1 verifica la primera vecindad, si valor es
 * 2 verifica la segunda vecindad
 *
 */
 ...
 *
 * @param ficheroName puntero a cadena con el nombre del archivo que contiene la
 * partida
 * @param filas puntero a entero donde se almacena la cantidad de filas del
 * tablero guardado
 * @param columnas puntero a entero donde se almacena la cantidad de columnas
 * del tablero guardado
 * @param conejosIniciales puntero a entero donde se almacenara la cantidad de
 * conejos iniciales
 * @param conejosVivos puntero a entero donde se alamacenara la cantidad de
 * conejos vivos en la partida guardada
 * @param nivel puntero a entero donde se almacenara el nivel de la partida
 * guardada
 * @param puntaje puntero a entero donde se almacenara el puntaje en la partida
 * guardada
 * @param trampolnes puntero a flotante donde se almacenara la cantidad de
 * trampolines guardadas
 *
 ...
 *
 * @param ficheroName puntero a cadena con el nombre del fichero donde se
 * guardara la partida
 ...
 * @param conejosIniciales entero con la cantidad inicial de conejos ingresada
 * por el usuario
 * @param conejosVivos entero con la cantidad de conejos vivos al momento de
 * llamar la funcion
...
int guardarPartida(char *ficheroName, char **tablero, const int filas,
                   ...
                   const int nivel, const int puntaje, const float trampolines);

/** @brief Genera una lista con las partidas disponibles en el directorio de
 * partidas.
 *
 * Va intentando abrir archivos consecutivos que sean posibles partidas
 * guardadas, cuando el primero de ellos no se puede abrir significa que no hay
 * mas.
 *
 ...
 *
 * @param ranking puntero a un arreglo de estructura itemRanking que contiene el
 * ranking
 * @param n entero correspondiente al tamano del ranking
 * @param elemento puntero a estructura con los datos que quieres ingresar al
 * ranking
 *
 * @return devuelve 0 si no puede ingresar al ranking o el numero de la posicion
 * en que quedo en el ranking
 */
 ...
 *
 * El ranking se guarda un archivo binario ya que hace mucho mas sencillo el
 * recuperar la informacion posteriormente.
 *
 * @param ranking puntero a un arreglo de estructura itemRanking que contiene el
 * ranking
 * @param n entero correspondiente al tamano del ranking
 ...
 *
 * @param ranking puntero a un arreglo de estructura itemRanking que contendra
 * el ranking
 * @param n entero correspondiente al tamano del ranking que se espera en el
 * archivo
 *
 * @return devuelve 1 si logro abrir el archivo y recuperar el rankin, 0 de lo
 * contrario
 */
 ...
 *
 * @param ranking puntero a un arreglo de estructura itemRanking que apunta a el
 * ranking
 * @param n entero correspondiente al tamano del ranking
 */
