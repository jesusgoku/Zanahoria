...
TipoNodoNombre *lista_vacia() { return NULL; }

int es_lista_vacia(TipoNodoNombre *lista) { return lista == NULL; }
...
int pertenece(TipoNodoNombre *lista, char *cadena) {
    ...
    for (aux = lista; aux != NULL; aux = aux->sig)
        if (!strcmp(aux->nombre, cadena)) return 1;
    return 0;
}
...
TipoNodoNombre *insertar_en_posicion(TipoNodoNombre *lista, int pos,
    ...
    for (i = 0, prev = NULL, aux = lista; i < pos && aux != NULL;
         i++, prev = NULL, aux = aux->sig)
        ;
    nuevo->sig = NULL;
    if (prev == NULL)
        lista = nuevo;
